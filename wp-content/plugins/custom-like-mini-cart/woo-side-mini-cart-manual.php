...
class Woo_Side_Mini_Cart_Manual {
    ...
    public function generate_cart_html() {
        ...
        foreach ( $items as $cart_item_key => $cart_item ) {
            ...
            // ✅ Поле ввода количества товара с кнопками `+` и `-`
            echo '<div class="woo-side-mini-cart__item-qty woocommerce-cart-form__quantity ">';
            echo '<button type="button" class="qty-decrease woocommerce-button" data-cart_item_key="' . esc_attr( $cart_item_key ) . '">−</button>';
            echo '<input type="number" min="1" step="1" class="cart-qty-input woocommerce-input" data-cart_item_key="' . esc_attr( $cart_item_key ) . '" value="' . esc_attr( $product_quantity ) . '">';
            echo '<button type="button" class="qty-increase woocommerce-button" data-cart_item_key="' . esc_attr( $cart_item_key ) . '">+</button>';
            echo '</div>';
        ...
        // ✅ Добавляем кнопки "Перейти в корзину" и "Оформить заказ"
        echo '<div class="woo-side-mini-cart__actions">';
        echo '<a class="button checkout wc-forward" href="' . esc_url( wc_get_checkout_url() ) . '">Оформить заказ</a>';
        echo '</div>';
